...
const ProfileSetting = (props) => {
    ...
    React.useEffect(() => {
        ...
        {props.forContractor ?
            axios.get('http://185.211.59.237/Account/GetCurrentContractorUser', {
                ...
                setSelectedProvince(res.data.user.province)
                setSelectedCity(res.data.user.city)
                let userGender = {...res.data.user}
                setSelectedGender(userGender.gender)
                setLoading(false)

                axios.get(`http://185.211.59.237/Account/Provinces/${defProvience.guid}/Cities`, {
                    headers: { Authorization: "Bearer " + cookies.token }
                }).then(res => {
    ...
    React.useEffect(() => {
        axios.get(`http://185.211.59.237/Account/Provinces/${selectedProvince.guid}/Cities`, {
            headers: { Authorization: "Bearer " + cookies.token }
        }).then(res => {
            setCities(res.data.cities)
            setCityLoading(false)
    ...
    const updateProfile = () => {
        let data = {
            ...
            genderGuid: selectedGender.value,
