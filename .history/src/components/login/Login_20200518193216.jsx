...
class LoginPage extends Component {
    ...
    registerStep1ClickHandler = () => {
        ...
        if (!this.state.registerFamlyValue) {
            this.setState({ registerFamlyValid: false, registerFamlyValid: 'الزامیست' })
        } else if (this.state.registerFamlyValue.length < 3) {
            this.setState({ registerFamlyValid: false, registerFamlyValid: 'کوتاه است' })
        } else {
            this.setState({ registerFamlyValid: true, registerFamlyValid: '' })
        }
        
        if (!this.state.registerPhoneNumberValue) {
            this.setState({ registerPhoneNumberValid: false, registerPhoneNumberValid: 'الزامیست' })
        } else if (
            !/^\d+$/.test(this.state.registerPhoneNumberValue)) {
            this.setState({ registerPhoneNumberValid: false, registerPhoneNumberValid: 'شماره موبایل معتبر نیست' })
        } else if(this.state.registerPhoneNumberValue.length !== 11) {
            this.setState({ registerPhoneNumberValid: false , registerPhoneNumberValid: 'شماره موبایل صحیح نیست' })
        } else {
            this.setState({ registerPhoneNumberValid: true , registerPhoneNumberValid: '' })
        }
    }
    ...
    registerStep2ClickHandler = () => {
        ...
        axios.post('http://185.94.97.164/api/Account/Authenticate', {
            ...
            if(res.data.token !== null) {
                ...
                toast('ثبت نام موفقیت آمیز بود', {type: toast.TYPE.ERROR})
            }
