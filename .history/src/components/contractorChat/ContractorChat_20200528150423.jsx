...
class ClientChat extends Component {
  ...
  componentDidMount () {
    ...
    axios.get(`http://Account/GetCurrentContractorUser`)
  ...
  removeOrdeerPageData = guid => {
    ...
    const dataTarget = orderPageData.find(data => {
      return orderPageData[data].orderGuid === guid
  })
    const updatedOrderPageData = orderPageData.filter(data => data[dataTarget])
