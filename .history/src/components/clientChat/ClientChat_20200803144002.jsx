...
class ClientChat extends Component {
  constructor (props) {
    ...
    this.state = {
      ...
      acceptModalAccepted: null,

      finishButtonShowState: null,

      ClientFinishJobModalHidden: true,

      profileSettingHidden: true,
      ...
      clickedOrdercontractorGuid: null,
  ...
  showPesronMessageModal = (reqGuid, contractorGuid, message, contractor, price, isAllowed, gender) => {
    const connection = this.state.connection
    const { cookies } = this.props
    const token = cookies.get('token')
    console.log(contractorGuid);
    ...
    this.setState({prevClickedOrderGuid: this.state.clickedOrderGuid}, () => {
      this.setState({
        ...
        clickedOrdercontractorGuid: contractorGuid,
    ...
    .then(res => {
      ...
      this.setState({
       ...
       if(res.data.allowingStatus === true) {
        this.setState({clientChatMainLoader: true})
        ...
        console.log(reqGuid);
            ...
            axios.get(`http://185.211.59.237/OrderRequest/GetFinishingStatus?orderRequestGuid: ${reqGuid}`,
            {
              headers: {Authorization: 'Bearer ' + token}
            }
            ).then(res => {
              console.log(res.data);
              this.setState({finishButtonShowState: res.data.finishingStatus})
            })
  ...
  declineOrderHandlerStep2 = () => {
    const { cookies } = this.props
    const token = cookies.get('token')

    this.setState({declineModalLoading: true})
  ...
  showClientDeclineModal = () => {
    this.setState({
      clientDeclineModalHidden: false
    })
  }

  hideClientDeclineModal = () => {
    this.setState({
      clientDeclineModalHidden: true
    })
  }
  ...
  showProfileSetting = () => {
    this.setState({
      profileSettingHidden: false
    })
  }

  hideProfileSettingModal = () => {
    this.setState({
      profileSettingHidden: true
    })
  }
  ...
  startChatHandler = contractor => {
    ...
    axios
      ...
      .then(res => {
        ...
        axios.get(`http://185.211.59.237/OrderRequest/GetFinishingStatus?orderRequestGuid=${guid}`,
        {
          headers: {Authorization: 'Bearer ' + token}
        }
        ).then(res => {
  ...
  renderFinishButtonByState = () => {
    if(!this.state.finishButtonShowState) {
        if(this.state.acceptModalAccepted === "در انتظار تایید") {
          return(
            <button
        ...
        }else if(this.state.acceptModalAccepted === "تایید شده") {
          return(
            <div className="chatbox-main-header-ignore-button-box">
            ...
            <button
              className='chatbox-main-header-ignore-button'
              onClick={this.showClientDeclineModal}>
                لغو کردن
            </button>
            </div>
          )
    ...
    if(this.state.orderRequestAcceptState === 1 || this.state.orderRequestAcceptState === 4) {
        return (
            ...
            {this.state.showResumePage ? (
              <ContractorResume hideResumePage={this.hideResumePage} contractorGuid={this.state.clickedOrdercontractorGuid} />
            ) : (
              <>
                ...
                >
                  ...
                  >
                    ...
                    {!this.state.clientChatMainLoader ?
                ...
                <div className='chatbox-main-content-sended-bottom'>
                  ...
                  <textarea
                      disabled={!this.state.finishButtonShowState ? true : false}
                      rows='4'
                      cols='50'
    ...
    return (
      <>
        ...
        <ClientAcceptModal
          ...
          hideClientAcceptModal={this.hideClientAcceptModal}
          declineOrderHandler={this.declineOrderHandler}
        ...
        <ClientDeclineModal
          hidden={this.state.clientDeclineModalHidden}
          hideClientDeclineModal={this.hideClientDeclineModal}
          declineOrderHandler={this.declineOrderHandlerStep2}
          disable={this.state.declineModalLoading}
        />
        ...
        <ProfileSetting
        hidden={this.state.profileSettingHidden}
        hideProfileSettingModal={this.hideProfileSettingModal}
        />
        ...
        <div className='chatboxkeeper'>
          <ClientChatSidebar
            ...
            showPesronMessageModal={(reqGuid, contractorGuid, message, contractor, price, isAllowed, gender) =>
              this.showPesronMessageModal(reqGuid, contractorGuid, message, contractor, price, isAllowed, gender)
            }
            clickedOrderGuid={this.state.clickedOrderGuid}
            showProfileSetting={this.showProfileSetting}
          />
