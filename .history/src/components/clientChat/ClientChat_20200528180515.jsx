...
class ClientChat extends Component {
  ...
  showPesronMessageModal = (reqGuid, message, contractor) => {
    const connection = this.state.connection
    const { cookies } = this.props
    const token = cookies.get('token')
  ...
  .then(res => {
      ...
      this.setState({
          AllowingStatusForChatRoom: res.data.acceptanceStatus
      })
       if(res.data.acceptanceStatus === true) {
      ...
      <>
        ...
        <div className='chatboxkeeper'>
          <ClientChatSidebar
            ...
            showPesronMessageModal={reqGuid, message, contractor =>
              this.showPesronMessageModal(reqGuid, message, contractor)
            }
          />
