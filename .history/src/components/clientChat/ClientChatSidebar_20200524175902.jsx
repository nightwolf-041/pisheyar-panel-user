...
function ClientChatSidebar(props) {
    ...
    const [orderRequestsForClient, setOrderRequestsForClient] = React.useState([])
    const [subOrderRequestsForClient, setSubOrderRequestsForClient] = React.useState([])
    ...
    React.useEffect(() => {
        axios.get('http://185.94.97.164/api/Order/GetClientOrders', {
            ...
            setOrderRequestsForClient(res.data.orders)
        })
    })

    const getSubOrederRequests = (guid) => {
        axios.get('http://185.94.97.164/api/Order/GetOrderRequestsForClient', {
            orderGuid: guid
        }, {
            headers: { Authorization: "Bearer " + cookies.token }
        }).then(res => {
            console.log(res.data);
            setOrderRequestsForClient(res.data)
        })
    }
        ...
        style={!props.hamburgerMenuToggle ? {right: '-500px'} : {right: 0}}>
            ...
            <PerfectScrollbar>
                ...
                <Accordion allowMultipleExpanded={true}>
                    {orderRequestsForClient.map((orderReq, index) => {
                        <AccordionItem key={index}>
                        <AccordionItemHeading >
                            <AccordionItemButton onClick={() => getSubOrederRequests(orderReq.orderGuid)}>
                        ...
                        <AccordionItemPanel>
                            {subOrderRequestsForClient.map(subOrderReq => {
