...
class ClientChat extends Component {
  ...
  componentDidUpdate = () => {
    const { cookies } = this.props
    const token = cookies.get('token')
    axios.get(`http://185.94.97.164/api/Account/GetCurrentClientUser`, {
  ...
  // sendClientGenderToTop = clientGender => {
  //   console.log(clientGender);
  //   this.setState({
  ...
  render () {
    console.log(this.state.clientGender);
    return (
      <>
        ...
        <div className='chatboxkeeper'>
          <ClientChatSidebar
            ...
            sendclientgendertotop={clientGender => this.setState({clientGender})}
            showPesronMessageModal={(reqGuid, message, contractor, price, isAllowed, gender) =>
