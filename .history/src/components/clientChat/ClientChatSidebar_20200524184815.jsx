...
function ClientChatSidebar(props) {
    ...
    const getSubOrederRequests = (guid) => {
        axios.get(`http://185.94.97.164/api/OrderRequest/GetOrderRequestsForClient?orderGuid=${guid}`, {
            headers: { Authorization: "Bearer " + cookies.token }
        }).then(res => {
            ...
            <PerfectScrollbar>
                ...
                <Accordion allowMultipleExpanded={true}>
                    {clientOrders.map((orderReq, index) => (
                        ...
                        <AccordionItemHeading >
                            <AccordionItemButton>
                                <div className="collapsible-trigger-division" onClick={() => getSubOrederRequests(orderReq.orderGuid)}>
                        ...
                        <AccordionItemPanel>
                            {subOrderRequestsForClient.map((subOrderReq, index) => (
                              ...
                              onClick={() => props.showPesronMessageModal(subOrderReq.orderGuid)}>
